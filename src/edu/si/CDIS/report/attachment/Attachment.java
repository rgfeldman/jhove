...
import edu.si.CDIS.report.DisplayFormat;
...
import edu.si.CDIS.report.Report;
...
public class Attachment extends Report  {
    ...
    public boolean createFileAndHeader (DisplayFormat displayFormat) {
        ...
        this.fileNameLoc =  CDIS.getSiHoldingUnit()+ "/rpt/CDISRPT-" + CDIS.getProjectCd().toUpperCase() + "-" + timeStamp + ".rtf";
    ...
    public List returnCdisMapList (DataSection section, String keyValue) {
        ...
        for (int s = 0; s < CDIS.getQueryNodeList().getLength(); s++) {
            ...
            if (sql.contains("?RPT_HOURS?")) {
                sql = sql.replace("?RPT_HOURS?", CDIS.getProperty("rptHours"));
            }
